...
```bash
# Criar arquivo docker-compose.yml (veja seção de Docker abaixo)
docker-compose up -d
...
npm run prisma:migrate         # Criar e executar migrations
npm run prisma:studio          # Abrir Prisma Studio (GUI)
npm run prisma:seed            # Executar seed scripts
```

## 📊 Prisma Studio

Para visualizar e gerenciar dados no banco de forma visual:

```bash
npm run prisma:studio
```

Abrirá uma interface em `http://localhost:5555`
